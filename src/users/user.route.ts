...
userRouter.post("/register", async (req: Request, res: Response) => {
    try {
        ...
        if (user){
             res.status(StatusCodes.BAD_REQUEST).json({ error: `This email has already beed registererd...`})
        }
...
userRouter.post("/login", async (req, res) => {
    try {
        ...
        if (!comparePassword) {
             res.status(StatusCodes.BAD_REQUEST).json({ error: `Password is incorrect..` });
        }
